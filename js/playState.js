...
function logCurrentStateCoin(game, coin) {
  ...
  for (const value of window.globalLevelState.coinCache.coins) {
    if (coin.x === value.x) {
      window.globalLevelState.coinCache.coins.splice(
        window.globalLevelState.coinCache.coins.indexOf(value),
        1
      );
      // console.log(value)
    }
    ...
    if (window.globalOtherHeros) {
      ...
      if (true /*messageEvent.channel === window.currentChannelName*/) {
        ...
        if (
          ...
          const frameDelta =
            parseInt(messageEvent.message.frameCounter) -
            otherplayer.lastKeyFrame;
          const initDelta =
          ...
          const frameDelay =
            ...
            window.frameCounter -
            initDelta +
            window.syncOtherPlayerFrameDelay;
          ...
          if (frameDelay > 0) {
            ...
            otherplayer.body.position.set(
              Number(messageEvent.message.x),
              Number(messageEvent.message.y)
            );
            ...
            if (otherplayer.totalRecvedFrames > 0) {
              const avgFrameDelay =
                otherplayer.totalRecvedFrameDelay /
                otherplayer.totalRecvedFrames;
          ...
          if (messageEvent.message.up === "down") {
            // If message equals arrow up, make the player jump with the correct UUID
          ...
          if (messageEvent.message.left === "down") {
            // If message equals arrow left, make the player move left with the correct UUID
            console.log("moving other player left (playState.js)");
            otherplayer.goingLeft = true;
          } else if (messageEvent.message.left === "up") {
            otherplayer.goingLeft = false;
          }
          if (messageEvent.message.right === "down") {
...
window.PlayState = {
  ...
  create() {
    ...
    makeOccupancyQuery(QUERY_UPDATE, true);
  ...
  _handleCollisions() {
    for (let i = 0; i < 2; i++) {
      // prevent collisions for pushing thru
      ...
      for (const uuid of window.globalOtherHeros.keys()) {
        ...
        this.game.physics.arcade.overlap(
          otherplayer,
          this.door,
          this._onOtherHeroVsDoor,
          this._canHeroEnterDoor,
          this
        );
      }
  ...
  _handleInput() {
    ...
    if (this.hero) {
      // Added this so we can control spawning of heros
      // Mobile Controls
      if (
        this.game.input.pointer1.x < 399 &&
        this.game.input.pointer1.y > 400 &&
      ...
      if (
        this.game.input.pointer1.x > 400 &&
        this.game.input.pointer1.y > 400 &&
        this.game.input.pointer1.isDown
      ) {
      ...
      if (
        this.game.input.activePointer.y < 401 &&
      ...
      if (
        ...
        (this.game.input.activePointer.x < 399 &&
          this.game.input.activePointer.y > 400 &&
          this.game.input.activePointer.isDown)
      ) {
        // move hero left
        this.hero.move(-1);
  ...
  _addOtherCharacter(uuid) {
    ...
    const playerText = this.game.add.text(
      this.hero2.body.x - 10,
      this.hero2.body.y - 550,
      "",
      { fill: "#000000", fontSize: "15px" }
    );
  ...
  _createHud() {
    ...
    const coinIcon = this.game.make.image(
      this.keyIcon.width + 7,
      0,
      "icon:coin"
    );
    const coinScoreImg = this.game.make.image(
