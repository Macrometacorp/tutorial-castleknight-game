...
function handleKeyMessages() {
  ...
  window.keyMessages.forEach((messageEvent) => {
    if (window.globalOtherHeros) { // If player exists
      if (true/*messageEvent.channel === window.currentChannelName*/) { // If the messages channel is equal to your current channel
        if (!window.globalOtherHeros.has(messageEvent.message.uuid)) { // If the message isn't equal to your uuid
          ...
          otherplayer.body.position.set(parseInt(messageEvent.message.x), parseInt(messageEvent.message.y)); // set the position of each player according to x y
          otherplayer.position.set(otherplayer.body.x, otherplayer.body.y);
          ...
          console.log("added other player to (playState.js)", otherplayer);
        ...
        if (messageEvent.message.x && window.globalOtherHeros.has(messageEvent.message.uuid)) { // If the message contains the position of the player and the player has a uuid that matches with one in the level
          ...
          const otherplayer = window.globalOtherHeros.get(messageEvent.message.uuid);
          const frameDelta = parseInt(messageEvent.message.frameCounter) - otherplayer.lastKeyFrame;
          const initDelta = otherplayer.initialRemoteFrame - otherplayer.initialLocalFrame;
          const frameDelay = (parseInt(messageEvent.message.frameCounter) - window.frameCounter) - initDelta + window.syncOtherPlayerFrameDelay;
          ...
          if (frameDelay > 0) {
            ...
            return;
          } else if (messageEvent.message.stopped === 'not moving') {

            otherplayer.body.position.set(Number(messageEvent.message.x), Number(messageEvent.message.y));
          ...
          if (messageEvent.message.up === 'down') { // If message equals arrow up, make the player jump with the correct UUID
            
            otherplayer.jump();
            otherplayer.jumpStart = Date.now();
          } else if (messageEvent.message.up === 'up') {
            otherplayer.jumpStart = 0;
          }
          if (messageEvent.message.left === 'down') { // If message equals arrow left, make the player move left with the correct UUID
            console.log("moving other player left (playState.js)");
            otherplayer.goingLeft = true;
          } else if (messageEvent.message.left === 'up') {
            otherplayer.goingLeft = false;
          }
          if (messageEvent.message.right === 'down') { // If message equals arrow down, make the player move right with the correct UUID
            console.log("moving other player right (playState.js)");
            otherplayer.goingRight = true;
          } else if (messageEvent.message.right === 'up') {
            otherplayer.goingRight = false;
          }
        }
      }
...
window.PlayState = {
  ...
  create() {
    ...
    console.log("||Attempt add me to occupancy");
    makeOccupancyQuery(QUERY_UPDATE, false);
    console.log("||After warning add me to occupancy");
    this._createHud();
  },
  ...
  _handleCollisions() {
    for (let i = 0; i < 2; i++) { // prevent collisions for pushing thru
      ...
      for (const uuid of window.globalOtherHeros.keys()) {
        ...
        this.game.physics.arcade.collide(otherplayer, this.platforms, this._onHitPlatform, null, this);
  ...
  _onHitPlatform(hero, platform) {
    //console.log("hero collision", hero.position.x, hero.position.y);
  },
  ...
  _goToNextLevel() {
    ...
    this.camera.onFadeComplete.addOnce(function () {
      ...
      if (this.level === 2) {
        window.createMyConnection(0);
      } else {
        window.createMyConnection(this.level + 1);
      }
    }, this);
  },
  ...
  _addOtherCharacter(uuid) {
    ...
    console.log('attempt _addOtherCharacter');
    this.hero2 = new window.Hero(this.game, 10, 10);
    this.hero2.body.bounce.setTo(0);
    this.hero2.lastKeyFrame = 0;
    const playerText = this.game.add.text(this.hero2.body.x - 10, this.hero2.body.y - 550, '', { fill: '#000000', fontSize: '15px' });
  ...
  _spawnCharacters(data) {
    console.log("spawned character");
    this.hero = new window.Hero(this.game, 10, 10);
    this.hero.body.bounce.setTo(0);
    const playerText = this.game.add.text(this.hero.body.x - 10, this.hero.body.y - 550, 'me', { fill: '#000000', fontSize: '15px' });
    playerText.anchor.set(0.5);
